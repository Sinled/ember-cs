module.exports = function(app) {
  ...
  var productsProductReviewsRouter = express.Router();
  ...
  productsProductReviewsRouter.get('/', function(req, res) {
    res.send({
      'products/product/reviews': REVIEWS
    });
  });

  productsProductReviewsRouter.post('/', function(req, res) {
    res.status(201).end();
  });

  productsProductReviewsRouter.get('/:id', function(req, res) {
    res.send({
  ...
  productsProductReviewsRouter.put('/:id', function(req, res) {
    res.send({
      'products/product/reviews': {
        id: req.params.id
      }
  ...
  productsProductReviewsRouter.delete('/:id', function(req, res) {
    res.status(204).end();
  });

  app.use('/api/products-product-reviews', productsProductReviewsRouter);
};
